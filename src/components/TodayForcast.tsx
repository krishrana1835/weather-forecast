...
function TodayForecast({lat, lon, data}: Props) {
  ...
  const [weatherData, setWeatherData] = useState<{time: string, weather: string, temperature: number}[]>([])
  ...
  return (
    <div className="w-100 component-bg todayforcast p-3">
      <p className="d-flex justify-content-start">TODAY'S FORECAST</p>
      <div className="forecast-row">
        {weatherData.map((info, index) => (
          <div key={index} className="forecast-item text-center">
            <p className="info m-0">{info.time.split(" ")[1].split(":")[0]}:00</p>
            <img src={`/${data.weather[0].main}${getDayOrNight(data)}.png`} alt="" height="50" width="50" className="my-2" />
